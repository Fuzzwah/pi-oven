...
| `cmd+y` | Copy nearest visible code block to clipboard; repeat to cycle through all blocks |
| `cmd+shift+x` | Archive current workspace (from tab or sidebar) |
| `cmd+u` | SCP upload: browse workspace destination → generates SCP command → copies to clipboard via OSC 52 |
| `cmd+a` | Copy local file to workspace: two-step browser (source on Mac, destination in repo) |
| `/btw <note>` (input) | Queue note as follow-up without interrupting the running agent |
...
5. **Add-project flow** — local dir, local repo, clone-from-URL with target-path prompt; tracker config UI per-project; GIT_ASKPASS shim wired up; default-branch sync with warn-and-proceed. Also: workspace setup script — if `workspace_setup.sh` exists in the repo root, run it automatically after worktree creation (useful for per-project dep install, env setup, etc.); logged to the workspace event log. Inline error dialog when the derived target directory already exists (`Directory '<name>' already exists`) instead of silent failure.
...
7. **Tracker event observability + merge cleanup** — tracker adapter PR methods (`createPullRequest`, `getPullRequest`, `listPullRequestChecks`, `deleteRemoteBranch`); webhook receiver with polling fallback; PR-merged detection drives auto-cleanup of the implementation workspace (worktree remove, remote branch delete, status closed). Agent-driven commit/push/PR-open flow works end-to-end against this. Squash-merge detection in archive preflight: if a branch has commits not in main's ancestry but the diff against main is empty, show "Squash-merged (N commits ahead, diff already in main)" at info severity rather than "Branch not merged" warning. Opt-in `use_gh_cli_merge_status = true` config flag adds a live PR state query via `gh` ("PR merged", "PR is open", "PR is a draft", "PR closed without merging"); git-based detection remains active as fallback.
...
10. **Polish** — branch-sync warnings surfaced in UI, tool-warnings banner, error toasts, theme crossfade animation, hot-reload of custom theme files, macOS appearance auto-switch, config file, `fsck` admin command, release-status indicators in tabs. Quit confirmation dialog: `Ctrl+Q` opens confirm with **Quit** pre-selected and **Cancel**; `Esc` dismisses; second `Ctrl+Q` also confirms — prevents accidental quit during active agent runs. `cmd+shift+X` archives the current workspace from anywhere (tab or sidebar), consistent with sidebar's existing `x` action. Sidebar project reordering: move-up/move-down keyboard actions, order persisted in server DB. `ctx%` status indicator in chat pane based on per-call token usage (`input + cache_creation + cache_read`) rather than cumulative totals; drops naturally after compaction. `cmd+a` copies a local Mac file to the current workspace: two-step browser (source file on Mac, destination directory in repo), transferred over WebSocket reusing the attachment upload binary-frame framing.
