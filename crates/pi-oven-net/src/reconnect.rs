...
pub struct ReconnectHandle {
    /// Send outgoing messages to the server (buffered while disconnected).
...
pub fn start(
    ...
    let (out_tx, out_rx) = mpsc::channel::<Msg>(64);
    ...
    'outer: loop {
        ...
        match Client::connect(&url, &shared_key, &client_version).await {
            Ok(handle) => {
                ...
                let conn_tx = handle.tx;
                let mut conn_rx = handle.rx;
                let mut close_fut = std::pin::pin!(handle.close);
                ...
                let close_info = loop {
                    tokio::select! {
                        ...
                        msg = conn_rx.recv() => {
                            match msg {
                                Some(m) => { let _ = in_tx.send(m).await; }
                                None => break None,
                            }
                        }
                        msg = out_rx.recv() => {
                            match msg {
                                Some(m) => { let _ = conn_tx.send(m).await; }
                                None => break 'outer,
                            }
                        }
                    }
